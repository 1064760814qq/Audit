...
const EchoooModule = artifacts.require("EchoooModuleTest");
...
contract("EchoooModule - Relayer", (accounts) => {
  let manager;
  ...
  describe("relay transactions", () => {
    ...
    it("should fail when the first param is not the wallet", async () => {
      ...
      await truffleAssert.reverts(
        manager.relay(module, "multiCall", [infrastructure, [transaction]], wallet, [owner], await utils.getGasPrice(), undefined, relayer),
    ...
    it("should fail when a wrong number of signatures is provided", async () => {
      const transaction = encodeTransaction(recipient, 10, ZERO_BYTES);
      await truffleAssert.reverts(
        manager.relay(module, "multiCall", [wallet.address, [transaction]], wallet, [owner, recipient], await utils.getGasPrice(), undefined, relayer),
  ...
  describe("refund", () => {
    ...
    it("should succeed when wallet is locked and refund is 0", async () => {
      ...
      const txReceipt = await manager.relay(
        ...
        wallet,
        [owner], undefined, undefined, undefined, true);
