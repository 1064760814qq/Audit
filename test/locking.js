...
contract("Locking", (accounts) => {
  ...
  describe("(Un)Lock by EOA guardians", () => {
    ...
    it("should be locked/unlocked by EOA guardians (relayed transaction)", async () => {
      await manager.relay(module, "lock", [wallet.address], wallet, [guardian1], await utils.getGasPrice(), undefined, relayer);
      let state = await module.isLocked(wallet.address);
      assert.isTrue(state);

      await manager.relay(module, "unlock", [wallet.address], wallet, [guardian1], undefined, undefined, undefined, true);
      state = await module.isLocked(wallet.address);
      assert.isFalse(state);
    });
  ...
  describe("(Un)Lock by Smart Contract guardians", () => {
    ...
    it("should fail to lock/unlock by Smart Contract guardians when signer is not authorized (relayed transaction)", async () => {
      await truffleAssert.reverts(manager.relay(module, "lock", [wallet.address], wallet, [nonguardian], await utils.getGasPrice(), undefined, relayer), "RM: Invalid signatures");
    });
  });
  ...
  describe("Unlocking wallets", () => {
    ...
    it("should not be able to unlock a wallet, locked by another feature", async () => {
      ...
      await manager.relay(module, "executeRecovery", [wallet.address, accounts[5]], wallet, [guardian1], await utils.getGasPrice(), undefined, relayer);
