...
contract("GuardianManager", (accounts) => {
  ...
  const guardian6 = "0x592916d0D7fcaec0A0A0504134364721Aafd5e87";
  const guardian7 = "0xF94e4af78b3f222fa0F8Be0F47b6Ec6960866E0b";
  const guardian8 = "0x6a18b5978B9F77C951D0309FACe33321b8710BB4";
  const guardian9 = "0x4d2E1A38d07Eadf5C62CfDaF93547DAe09F1EF83";
  ...
  describe("Adding Guardians", () => {
    describe("EOA Guardians", () => {
      ...
      it("add 2 EOA Guardians", async () => {
        await module.addGuardian(wallet.address, [guardian4,guardian5], { from: owner });
        await utils.increaseTime(30);
        await module.confirmGuardianAddition(wallet.address, [guardian4,guardian5]);
        let count = (await guardianStorage.guardianCount(wallet.address)).toNumber();
        let active = await module.isGuardian(wallet.address, guardian4);
        assert.isTrue(active, "second guardian should be active");
        assert.equal(count, 3, "3 guardians should be active");

      })

      it("add 3 EOA Guardians", async () => {
        await module.addGuardian(wallet.address, [guardian2,guardian4,guardian5], { from: owner });
        await utils.increaseTime(30);
        await module.confirmGuardianAddition(wallet.address, [guardian2,guardian4,guardian5]);
        let count = (await guardianStorage.guardianCount(wallet.address)).toNumber();
        let active = await module.isGuardian(wallet.address, guardian4);
        assert.isTrue(active, "second guardian should be active");
        assert.equal(count, 4, "4 guardians should be active");

      })

      it("add 4 EOA Guardians", async () => {
        await module.addGuardian(wallet.address, [guardian4,guardian5,guardian6,guardian7], { from: owner });
        await utils.increaseTime(30);
        await module.confirmGuardianAddition(wallet.address, [guardian4,guardian5,guardian6,guardian7]);
        let count = (await guardianStorage.guardianCount(wallet.address)).toNumber();
        let active = await module.isGuardian(wallet.address, guardian4);
        assert.isTrue(active, "second guardian should be active");
        assert.equal(count, 5, "5 guardians should be active");
      })
      ...
      it("should add many Guardians", async () => {
        ...
        let active;
        for (let i = 2; i <= 3; i += 1) {
          await manager.relay(module, "addGuardian", [wallet.address, [guardians[i - 1]]], wallet, [owner], await utils.getGasPrice(), undefined, relayer);
          if (i > 1) {
            await utils.increaseTime(30);
            await manager.relay(module, "confirmGuardianAddition", [wallet.address, [guardians[i - 1]]], wallet, [], await utils.getGasPrice(), undefined, relayer);
          }
          count = (await guardianStorage.guardianCount(wallet.address)).toNumber();
    ...
    describe("Smart Contract Guardians", () => {
      ...
      it("should let the owner add a Smart Contract guardian", async () => {
        await manager.relay(module, "addGuardian", [wallet.address, [guardianWallet2.address]], wallet, [owner], await utils.getGasPrice(), undefined, relayer);
        await utils.increaseTime(30);
  ...
  describe("Revoking Guardians", () => {
    ...
    it("should revoke a guardian (relayed tx)", async () => {
      await manager.relay(module, "revokeGuardian", [wallet.address, guardian1], wallet, [owner], await utils.getGasPrice(), undefined, relayer);
      let count = (await guardianStorage.guardianCount(wallet.address)).toNumber();
      let active = await module.isGuardian(wallet.address, guardian1);
      assert.isTrue(active, "the revoked guardian should still be active during the security period");
      assert.equal(count, 2, "the revoked guardian should go through a security period");
      ...
      await utils.increaseTime(30);
      await manager.relay(module, "confirmGuardianRevokation", [wallet.address, guardian1], wallet, [], await utils.getGasPrice(), undefined, relayer);
  ...
  describe("Cancelling Pending Guardians", () => {
    ...
    it("owner should be able to cancel pending revokation of guardian (relayed)", async () => {
      await module.revokeGuardian(wallet.address, guardian1, { from: owner });
      await manager.relay(module, "cancelGuardianRevokation", [wallet.address, guardian1], wallet, [owner], await utils.getGasPrice(), undefined, relayer);
