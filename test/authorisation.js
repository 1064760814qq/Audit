...
contract("Authorisation", (accounts) => {
  ...
  async function setupWallet() {
    ...
    const txReceipt = await manager.relay(module, "addToWhitelist", [wallet.address, dappRegistry.address], 
      wallet, [owner], await utils.getGasPrice(), ETH_TOKEN, relayer);
    const { success, error } = await utils.parseRelayReceipt(txReceipt);
  ...
  describe("call (un)authorised contract", () => {
    ...
    it("should call authorised contract when filter passes (echooo registry)", async () => {
  ...
  describe("enable/disable registry for wallet", () => {
    ...
    it("should allow disabling the Echooo Registry", async () => {
      ...
      assert.equal(await dappRegistry.isEnabledRegistry(wallet.address, 0), true, "Echooo Registry should be enabled by default");
      await assertToggle(false);
      assert.isFalse(await dappRegistry.isEnabledRegistry(wallet.address, 0), "Echooo Registry should be disabled");
      await assertToggle(true);
      assert.isTrue(await dappRegistry.isEnabledRegistry(wallet.address, 0), "Echooo Registry isn't re-enabled");
    });
  ...
  describe("add/remove dapp", () => {
    ...
    it("should not allow non-owner to add authorisation to the Echooo registry", async () => {
      await truffleAssert.reverts(
