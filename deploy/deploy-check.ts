import hre, { ethers } from "hardhat";
import dotenv from "dotenv";
import { BigNumber } from "ethers";
import { getEthAddressFromKMS, KMSSigner } from '@rumblefishdev/eth-signer-kms';

dotenv.config();

const baseBalance = ethers.BigNumber.from("1000000000000000000");
const minBalance:BigNumber = ethers.BigNumber.from(3).mul(baseBalance).div(10);

export async function main() {
  const ethProxyBytecode = "";
    const Proxy = require(`../artifacts/contracts/wallet/Proxy.sol/Proxy.json`);
    if (Proxy.bytecode != ethProxyBytecode) {
      console.log("bytecode not match eth");
      throw new Error("bytecode not match eth")
    }
    
  //check signer have enough balance
   const [signer] = await hre.ethers.getSigners();

   const infrastructureBalance = await hre.ethers.provider.getBalance(signer.address)
    if (infrastructureBalance.lt(minBalance)) {
        console.log(" not enough balance ");
        console.log("infrastructureBalance:", ethers.utils.formatEther(infrastructureBalance));
        console.log("minBalance:", ethers.utils.formatEther(minBalance));
        throw new Error("not enough balance");
    }
 }
 
 if (require.main === module) {
   main()
   .then(() => process.exit(0))
   .catch((error) => {
     console.error(error);
     process.exit(1);
   });
 }