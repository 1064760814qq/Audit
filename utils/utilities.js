...
const utilities = {
  ...
  getChainId: async () => {return await web3.eth.getChainId();},
  ...
  waitForBlock: async(begin) => {
     while((await web3.eth.getBlockNumber()) == begin) {
        console.log("No new block is generated yet, sleep for 1 second!");
  ...
  increaseTime: async (seconds) => {
    ...
    if (!client.includes("TestRPC") && !client.includes("HardhatNetwork")) {
      //console.warn("Client is not ganache-cli or hardhat network and cannot forward time");
      console.log(`sleeping for ${seconds} seconds!`);
      await utilities.sleep(seconds);
      // In order for the increase time to be visible on the blockchain by
      // the contract, we need to wait until a new block is mined!
      await utilities.waitForBlock(await web3.eth.getBlockNumber());
    } else {
  ...
  initNonce: async (wallet, module, manager, securityPeriod, _gasPrice = 0, _refundAddress = ethers.constants.AddressZero) => {
    ...
    await manager.relay(
      ...
      wallet,
      [owner], _gasPrice, undefined, _refundAddress);
  ...
  require_test_artifacts: async (c_name) => {
    const con = require(`../build-test/contracts/${c_name}.json`);
    const art = TruffleContract(con);
    await art.setProvider(web3.currentProvider);
    art.defaults({ from: (await web3.eth.getAccounts())[0] });
    return art;
  },

  require_tl_artifacts: async (c_name) => {
    const con = require(`../../echooo_trustlists/artifacts/contracts/${c_name}.sol/${c_name}.json`);
    const art = TruffleContract(con);
    await art.setProvider(web3.currentProvider);
    art.defaults({ from: (await web3.eth.getAccounts())[0] });
    return art;
