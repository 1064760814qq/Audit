 ...
 * @author skywalker @ Echooo Lab - <bob@echooo.xyz>
 */
contract WalletFactory is Managed {

    address constant internal ETH_TOKEN = address(0);

    IModuleRegistry immutable registry;
     ...
     */
    constructor(address _walletImplementation, address _guardianStorage, address _refundAddress, IModuleRegistry _registry) {
        ...
        require(address(_registry) != address(0), "WF: empty registry address");
    ...
    */
        ...
        returns (address _wallet)
    {
        ...
        bytes32 newsalt = newSalt(_salt, _owner, _guardian);
     ...
     * @param _guardian The guardian address.
     */
    function newSalt(bytes20 _salt, address _owner, address _guardian) internal pure returns (bytes32) {
        return keccak256(abi.encodePacked(keccak256(abi.encodePacked(_owner, _guardian)), _salt));
    }
    ...
    function validateInputs(address _owner, address[] calldata _modules, address _guardian) internal view {
        require(registry.isRegisteredModule(_modules),"WF: modules not registry");
