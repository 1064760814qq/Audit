 ...
 * @author Skywalker - <bob@echooo.xyz>
 */
    ...
    function isMultiCallAuthorised(address _wallet, Call[] calldata _transactions) external view returns (bool) {
        for(uint i = 0; i < _transactions.length; i++) {
            ...
            if (
                ...
                (!isWhitelisted(_wallet, spender) && authorised(_wallet, spender, _transactions[i].to, _transactions[i].data) == MAX_UINT)
            ) {
                return false;
            }
        }
    ...
    function multiCallAuthorisation(address _wallet, Call[] calldata _transactions) external view returns (uint256[] memory registryIds) {
        ...
        for(uint i = 0; i < _transactions.length; i++) {
            ...
            if (spender != _transactions[i].to && _transactions[i].value != 0) {
                ...
                registryIds[i] = authorised(_wallet, spender, _transactions[i].to, _transactions[i].data);
            }
        }
    }

    function authorised(address _wallet, address _spender, address _to, bytes calldata _data) internal view returns (uint256) {
